...
//                     final visibleCategories = controller.categories.where((
//                       cat,
//                     ) {
//                       if (cat == 'All') return true;

//                       return controller.recipes.any(
...
//   // 👈 ADD: ticks every 30s so the "x ago" text updates live without
//   // needing any other state change (like/save) to trigger a rebuild.
//   Timer? _agoTicker;
...
//     // 👈 ADD: refresh the relative "x ago" label periodically. 30s keeps
//     // "just now" → "1m ago" transitions feeling live without excessive
//     // rebuilds (a full-precision per-second timer isn't needed for a
//     // minute-granularity label).
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ── Category chips ──
                  // `controller.categories` already returns ONLY the chips
                  // that are Firestore-verified to have at least 1 public
                  // recipe right now ("All" is always included). It is
                  // realtime: reading `categoryHasData` inside this Obx
                  // means the chip row updates the instant a recipe is
                  // published/unpublished — no manual refresh needed, and no
                  // empty tab is ever shown.
                  Obx(() {
                    final selected = controller.selectedCategory.value;
