...
class NutritionEstimator {
  ...
  static NutritionModel estimate({
    ...
    int unknownCount = 0; // 👈 Count unknown ingredients
    ...
    for (final raw in ingredientLines) {
      ...
      final isKnown = food.keywords.first != '*unknown*';

      if (!isKnown) {
        unknownCount++;
        // Skip this ingredient entirely (don't add to totals)
        continue;
      }
    ...
    // If ALL ingredients are unknown, return empty model
    if (unknownCount > 0 && kcal <= 0) {
      return NutritionModel.empty; // 👈 Return empty model
    }
  ...
  static _Parsed? _parse(String raw) {
    ...
    if (s.isNotEmpty && _unicodeFractions.containsKey(s[0])) {
      ...
      final m =
          RegExp(r'^(\d+)\s+(\d+)\s*/\s*(\d+)').firstMatch(s) ?? // 1 1/2
  ...
  static const List<_Food> _foods = [
    ...
    _Food(
      ['vegetable oil', 'canola', 'sunflower oil', 'oil'],
      884,
      0,
      0,
      100,
    ...
    _Food(
      ['curry paste', 'curry powder', 'masala'],
      150,
      3,
      12,
      9,
    ...
    _Food(
      ['greek yogurt', 'yoghurt', 'yogurt', 'curd'],
      59,
      10,
      3.6,
      0.4,
    ...
    _Food(
      ['almond', 'cashew', 'walnut', 'nut', 'pecan'],
      579,
      21,
      22,
      50,
    ...
    _Food(
      ['coriander', 'cilantro', 'parsley', 'basil', 'mint', 'herb'],
      23,
      2,
      4,
      0.5,
    ...
    _Food(
      ['cumin', 'turmeric', 'paprika', 'spice', 'cinnamon', 'pepper'],
      375,
      18,
      44,
      22,
  ...
  static _Food _lookup(String name) {
    ...
    if (n.isEmpty) return _fallbackUnknown;
    ...
    // 👇 Instead of returning fallback with nutrition data,
    // return a special "unknown" marker.
    return _fallbackUnknown;
  }

  // Add this constant
  static const _Food _fallbackUnknown = _Food(
    ['*unknown*'],
    0, // kcal → 0
    0, // protein → 0
    0, // carbs → 0
