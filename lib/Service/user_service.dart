...
class UserService {
  ...
  static Future<void> ensureUsername() async {
    ...
    final existing = (doc.data()?['username'] as String?)?.trim() ?? '';
    if (existing.isNotEmpty) return;

    final base = _slug(user.email?.split('@').first ??
        user.displayName ??
        'chef');
    var candidate = base;
    var n = 0;
    ...
    await _userRef(user.uid).set(
        {'username': candidate}, SetOptions(merge: true));
  }
  ...
  static Future<String?> setUsername(String raw) async {
    ...
    final current = (await _userRef(user.uid).get()).data()?['username'] ?? '';
