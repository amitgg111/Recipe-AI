...
class AiTranslationService {
  ...
  static Future<void> _ensureModelDownloaded(String bcp) {
    ...
    final future = () async {
      ...
      if (downloaded) {
        print('✅ Model already downloaded: $bcp');
        return;
      }
      print('⬇️ Downloading ML Kit model: $bcp');

      if (!await manager.isModelDownloaded(bcp)) {
        await manager.downloadModel(bcp, isWifiRequired: false);
      }
      print('✅ ML Kit model downloaded: $bcp');
    }();
  ...
  /// call (like the one in your print) runs on the UI thread.
  static Future<void> prewarmExistingRecipesTranslation() async {
    ...
    if (user == null) return; // printged out user pase recipes j nathi
    ...
    try {
      ...
      print(
        '🌐 Prewarm: found ${snap.docs.length} existing recipe(s) to cache',
      );
      ...
      print('✅ Prewarm: existing recipes cache warm');
    } catch (e) {
      print('❌ Prewarm existing recipes failed: $e');
    }
  }
  ...
  static Future<void> prewarmLanguage(
    ...
    try {
      ...
      print('❌ Prewarm: model download failed [$code]: $e');
      return;
    }
    final user = knownUser ?? FirebaseAuth.instance.currentUser;

    if (user == null) {
      print('⏭️ Prewarm[$code]: signed out — model ready, no text to cache');
      return;
    }
    ...
    try {
      ...
      print('✅ Prewarm[$code]: cache warm, ${cache.length} entries');
      _markPrewarmed(bcp);
    } catch (e) {
      print('❌ Prewarm[$code] failed: $e');
    }
  }
  ...
  static Future<void> prepareAlternateLanguages() async {
    ...
    print('🌍 Country: ${LanguageService.resolvedCountryCode}');
    print('🌐 Alternate languages to prepare: $languages');
    ...
    print('✅ Alternate language preparation completed');
  }
  ...
  static Future<bool> _prepare(TranslateLanguage target, String bcp) async {
    ...
    try {
      ...
      print('❌ _prepare failed for $bcp: $e');
      return false;
    } finally {
  ...
  static Future<bool> activateLanguage(String code) async {
    ...
    try {
      ...
      if (!downloaded) {
        print('❌ Language model NOT preloaded: $code ($bcp)');
        return false;
      }
      ...
      print('✅ Language model already downloaded: $code ($bcp)');

      return true;
    } catch (e) {
      print('❌ activateLanguage error [$code]: $e');
      return false;
    }
  }
  ...
  static Future<void> preloadLanguageModel(String code) async {
    ...
    try {
      ...
      print('🌐 Preload check: $code ($bcp) downloaded=$downloaded');

      if (!downloaded) {
        print('⬇️ Background downloading: $code ($bcp)');

        await manager.downloadModel(bcp, isWifiRequired: false);

        print('✅ Background download completed: $code');
      }
    } catch (e) {
      print('❌ Background preload failed [$code]: $e');
    }
  }
  ...
  static Future<void> preloadDeviceLanguages() async {
    ...
    print('🌍 Resolved country: ${LanguageService.resolvedCountryCode}');
    print('🌐 Models to preload: $languages');

    for (final language in languages) {
      print('🔄 Preparing language model: $language');
      await preloadLanguageModel(language);
    }

    print('✅ Device language preload completed');
  }

  static Future<void> prepareSplashLanguages() async {
    final languages = LanguageService.deviceLanguages;

    print('🚀 Splash language preparation started');
    print('🌍 Country: ${LanguageService.deviceCountryCode}');
    ...
    for (final language in languages) {
      ...
      print('✅ Model ready: $language');
    }

    print('🎉 Splash language preparation completed');
  }
  ...
  static Future<String> translateToEnglish(String? text) async {
    ...
    try {
      ...
      if (sourceLanguage == null) {
        print('⚠️ Unsupported source language: $detectedLanguage');
        return input;
      }
      ...
      print('❌ translateToEnglish error: $e');
  ...
  static Future<String> detectDominantLanguage(List<String> sampleTexts) async {
    ...
    if (scriptLang != null) {
      ...
      print('🌍 Script-detected language: $code');
      return code;
    }
    try {
      final detected = await _languageIdentifier.identifyLanguage(combined);
      print('🌍 Dominant language detected: $detected (sample: "$combined")');
      return detected;
    } catch (e) {
      print('❌ detectDominantLanguage error: $e');
      return 'und';
    }
  }
  ...
  static Future<OnDeviceTranslator?> prepareTranslatorFor(
    ...
    if (sourceLanguage == null) {
      print('⚠️ Unsupported source language: $languageCode');
      return null;
    }
    ...
    try {
      ...
      print('❌ Model download failed for $languageCode: $e');
      return null;
    }
  ...
  //   print('🌐 Preparing ALL supported languages: $languages');
  ...
  //   print('✅ All supported languages prepared');
  // }
  ...
  static Future<void> prepareAllSupportedLanguages() async {
    ...
    if (user == null) {
      print(
        '⏭️ No signed-in user — downloading models only, '
        'recipe cache skipped',
      );
    }

    for (final language in languages) {
      try {
        print('⬇️ Preparing: $language');

        await prewarmLanguage(language, knownUser: user);

        print('✅ Prepared: $language');
      } catch (e) {
        print('❌ Failed [$language]: $e');
      }
    }
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // COOKBOOK NAME TRANSLATION — purely additive, nothing above is touched.
  // Thin wrappers around the existing translate()/translateList() so
  // callers (e.g. CookbookController) don't need to know the cache
  // mechanics, and cookbook names share the SAME per-language cache as
  // recipe text (no separate storage needed).
  // ═══════════════════════════════════════════════════════════════════════

  /// Translates a single cookbook name (or any short UI label) into the
  /// current language and caches it. Falls back to the original name if
  /// translation isn't available/needed (English, unsupported language,
  /// offline, etc.) — same behavior as [translate].
  static Future<String> translateCookbookName(String? name) {
    return translate(name);
  }

  /// Batch variant for a full list of cookbook names — thin wrapper around
  /// [translateList]. Prefer this over calling [translateCookbookName] in a
  /// loop: it reuses one translator instance and does one disk write for
  /// the whole batch instead of one per name.
  static Future<List<String>> translateCookbookNames(List<String> names) {
    return translateList(names);
  }
}
