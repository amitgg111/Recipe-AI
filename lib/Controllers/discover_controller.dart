...
class DiscoverController extends GetxController {
  ...
  List<DiscoverRecipe> get filteredRecipes {
    ...
    switch (sel) {
      case 'All':
        // Newest-first only — see final sort below.
        break;
      case 'Quick & Easy':
        // 30 minutes or less (total → cook → prep, whichever is available).
        list = list.where((r) {
      ...
      case 'Desserts':
        ...
        break;
      default:
        // Meal / course categories AND onboarding-cuisine chips — the chip
        // labels are English, so match the recipe's ENGLISH category /
        // cuisine / title (filter* fields).
        if (sel.isNotEmpty) {
          final q = sel.toLowerCase();
          list = list
              .where(
    ...
    // Newest first — always, regardless of the selected chip. No cuisine
    // priority is applied.
    list.sort((a, b) {
      final aDate = a.createdAt ?? DateTime.fromMillisecondsSinceEpoch(0);
      final bDate = b.createdAt ?? DateTime.fromMillisecondsSinceEpoch(0);
