...
/// Carries BOTH the canonical English fields (always saved, unchanged) AND
/// the untouched original-language fields + the detected language code, so
/// the recipe owner can be shown their recipe in the language they typed it
/// in, while everyone else sees the English canonical copy translated into
/// their own selected app language (see RecipeLocalizer).
class _RecipeFieldsTranslationOutcome {
  final _TranslatedRecipeFields english;
  final _TranslatedRecipeFields original;
  final String languageCode;

  _RecipeFieldsTranslationOutcome({
...
class RecipeEditorController extends GetxController {
  ...
  Future<_RecipeFieldsTranslationOutcome> _translateFieldsToEnglish() async {
    ...
    if (detected == 'en' || detected == 'und') {
      ...
      // `original` and `english` are the same object here, so the owner and
      // everyone else see identical text.
      return _RecipeFieldsTranslationOutcome(
        english: fallback,
        original: fallback,
        languageCode: 'en',
      );
    ...
    if (translator == null) {
      ...
      // Couldn't translate — English falls back to the as-typed text too,
      // but the language code is still recorded for a possible later retry.
      return _RecipeFieldsTranslationOutcome(
        english: fallback,
        original: fallback,
    ...
    try {
      ...
      final englishFields = _TranslatedRecipeFields(
      ...
      // `fallback` holds the untouched as-typed fields — returned as the
      // original-language snapshot so the owner sees their recipe exactly
      // as they wrote it.
      return _RecipeFieldsTranslationOutcome(
  ...
  Future<void> saveRecipe() async {
    try {
      ...
      final translationOutcome = await _translateFieldsToEnglish();
      final translated = translationOutcome.english;
      final originalFields = translationOutcome.original;
      ...
      final recipeData = {
        ...
        // The owner sees the recipe in the language they typed it in
        ...
        "original": {
          ...
          "prepTime": originalFields.prepTime,
          "cookTime": originalFields.cookTime,
          "totalTime": originalFields.totalTime,
          "servings": originalFields.servings,
          "category": originalFields.category,
          "cuisine": originalFields.cuisine,
          "keywords": originalFields.keywords,
          "ingredients": originalFields.ingredients,
          "instructions": originalFields.instructions,
          "ingredientSections": originalFields.ingredientSections
              .map((s) => s.toMap())
              .toList(),
          "instructionSections": originalFields.instructionSections
              .map((s) => s.toMap())
              .toList(),
        },
